...
//
// Ce que le parcours PROUVE (journal `[VITRINE-LOG]`, une capture par étape prise
...
Future<void> _step(WidgetTester tester, String name,
  ...
  print('[VITRINE-LOG] ${DateTime.now().toIso8601String()} $name');
  await tester.pump(hold);
}

void _ok(String what, bool cond, {String? detail}) {
  print('[VITRINE-R] ${cond ? 'OK ' : 'KO '} $what${detail == null ? '' : ' — $detail'}');
}
...
void main() {
  ...
  setUpAll(() async {
    ...
    try {
      ...
      debugPrint('[VITRINE-LOG] Firebase : $e');
    }
    setupDependencies();
    if (!Get.isRegistered<ThemeController>()) Get.put(ThemeController(), permanent: true);
  });

  testWidgets('585 — vitrine ($kRole)', (tester) async {
    ...
    FlutterError.onError = (d) {
      print('[VITRINE-LOG] EXCEPTION FLUTTER: ${d.exceptionAsString()}\n${d.stack}'.split('\n').take(12).join('\n'));
      prevOnError?.call(d);
    };
    try {
      await _run(tester);
    } catch (e, st) {
      print('[VITRINE-LOG] EXCEPTION TEST: $e\n${st.toString().split('\n').take(10).join('\n')}');
      rethrow;
    }
  });
}
...
Future<void> _run(WidgetTester tester) async {
    ...
    for (var i = 0; i < 40 && !navWrapperMounted.value; i++) {
      ...
      if (dlg.evaluate().isNotEmpty) {
        ...
        if (buttons.evaluate().length >= 2) {
          ...
          debugPrint('[VITRINE-LOG] fenetre des notifications : « Plus tard »');
          await tester.pump(const Duration(seconds: 1));
        }
      }
    ...
    if (find.byKey(const ValueKey<String>('pawmap_coach')).evaluate().isNotEmpty) {
      ...
      print('[VITRINE-LOG] croix: ${close.evaluate().length} · rect=${close.evaluate().isEmpty ? '-' : tester.getRect(close)}');
    ...
    await shot('01_pawmap_bas');
    final sheet = find.byType(DraggableScrollableSheet);
    final ctl = tester.widget<DraggableScrollableSheet>(sheet).controller!;
    ctl.jumpTo(tester.widget<DraggableScrollableSheet>(sheet).maxChildSize);
    await tester.pump(const Duration(seconds: 1));
    await shot('02_feuille_haut');
    final lv = find.descendant(of: sheet, matching: find.byType(Scrollable)).first;
    for (var i = 0; i < 8; i++) {
      await tester.drag(lv, const Offset(0, -500));
      await tester.pump(const Duration(milliseconds: 400));
    }
    await shot('03_feuille_haut_fin_dock');
    ctl.jumpTo(tester.widget<DraggableScrollableSheet>(sheet).minChildSize);
    await tester.pump(const Duration(seconds: 1));
    final subs = find.byKey(const ValueKey<String>('pawmap_btn_subs'));
    ctl.jumpTo(0.46);
    await tester.pump(const Duration(seconds: 1));
    if (subs.evaluate().isNotEmpty) {
      await tester.tap(subs);
      await tester.pump(const Duration(seconds: 1));
      await shot('04_mes_abonnements');
      Get.back();
      await tester.pump(const Duration(seconds: 1));
    }
    ctl.jumpTo(tester.widget<DraggableScrollableSheet>(sheet).minChildSize);
    requestedTab.value = 4;
    await tester.pump(const Duration(seconds: 3));
    await shot('05_profil');
    final fr = find.byKey(const ValueKey<String>('profile_friends_row'), skipOffstage: false);
    if (fr.evaluate().isNotEmpty) {
      await tester.scrollUntilVisible(fr, 300,
          scrollable: find.byType(Scrollable).hitTestable().first);
      await tester.pump(const Duration(milliseconds: 600));
      await shot('05b_profil_mes_amis');
    } else {
      print('[VITRINE-LOG] ligne Mes amis introuvable');
    }
    if (kRole != 'owner') {
      Get.to(() => MyRatesScreen(role: kRole));
      await tester.pump(const Duration(seconds: 3));
      ...
      if (dd.evaluate().isNotEmpty) {
        ...
        await tester.pump(const Duration(seconds: 1));
        await shot('06_mes_tarifs_menu_devise');
        Get.back();
        await tester.pump(const Duration(milliseconds: 600));
      } else {
        print('[VITRINE-LOG] pas de menu deroulant dans Mes tarifs');
      }
      Get.back();
      await tester.pump(const Duration(seconds: 1));
    }
