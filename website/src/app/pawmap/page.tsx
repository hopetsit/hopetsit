  ...
  return (
    <div className="bg-white">
      ...
      <div className="mx-auto max-w-4xl px-4 pb-12 pt-20 text-center md:pt-28">
        ...
        <PawMapHeroBadge />
        <span className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#FAF1EC] px-3.5 py-1.5 text-xs font-semibold text-[#6E4F48]">
          🌍 {t("hero_badge")}
        </span>
        <h1 className="mt-5 font-display text-[2.75rem] font-bold leading-[1.05] tracking-[-0.03em] text-[#231715] md:text-6xl">
          {t("pawmap_title")}
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-lg text-[#6E4F48] md:text-xl">{t("pawmap_sub")}</p>
      ...
      <div className="mx-auto max-w-5xl px-4 py-24">
        ...
        <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {cats.map((c) => (
            <div key={c.label} className="flex items-center gap-3 rounded-[18px] bg-[#FAF1EC] p-4">
        ...
        {/* ── 4. COMMENT ÇA MARCHE ── */}
        <h2 className="mt-24 text-center font-display text-3xl font-bold tracking-[-0.02em] text-[#231715] md:text-5xl">
          {t("nav_how")}
        </h2>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {steps.map((s) => (
            <div key={s.n} className="rounded-[24px] bg-[#FAF1EC] p-7">
              <span className="font-display text-sm font-semibold tracking-widest text-owner">{s.n}</span>
              <p className="mt-3 text-[15px] leading-relaxed text-[#231715]">{s.body}</p>
            </div>
          ))}
        </div>

        {/* ── 5. PAWPREMIUM ── */}
        <section className="mt-24 flex flex-col items-center gap-8 rounded-[28px] bg-[#231715] p-10 text-center md:flex-row md:p-14 md:text-left">
