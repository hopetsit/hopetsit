  ...
  return (
    <section className={compact ? "py-12" : "bg-white py-24"}>
      <div className="mx-auto max-w-6xl px-4">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-center text-lg leading-relaxed text-[#6E4F48]">
          {t("sub_sub")}
        </p>

        <div className="mt-12 grid gap-4 md:grid-cols-3">
          {plans.map((p) => (
            <article key={p.key} className="flex flex-col rounded-[24px] bg-[#FAF1EC] p-7">
              <div className="flex items-center gap-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.logo} alt="" width={56} height={56} className="h-14 w-14 shrink-0" />
                <div>
                  <h3 className="font-display text-xl font-semibold leading-tight text-[#231715]">{p.name}</h3>
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="rounded-[16px] bg-white p-4">
                  ...
                  <p className="mt-1 text-[15px] leading-relaxed text-[#231715]">{p.plus}</p>
                </div>
              </div>

              <Link href="/boutique" className="mt-auto pt-6 text-sm font-semibold text-[#231715] hover:underline">
                {t("sub_cta")} {p.name} →
              </Link>
            </article>
          ))}
