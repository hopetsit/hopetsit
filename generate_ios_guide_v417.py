...
OUT = "C:/Users/Usuario/Downloads/HoPetSit_iOS_Build_Guide_v23.1.417.pdf"
doc = SimpleDocTemplate(OUT, pagesize=A4, leftMargin=1.8*cm, rightMargin=1.8*cm,
                        topMargin=1.8*cm, bottomMargin=1.8*cm, title="HoPetSit iOS v417")
...
f = []
f.append(Paragraph("HoPetSit - iOS Build Guide v23.1.417 (Build 417)", H1))
f.append(Paragraph("Nouveautes depuis v416 : suivi en direct qui survit a l'arriere-plan (et au swipe-kill cote Android), + rappels APNs toujours valables. Le backend et le site sont deja en ligne (push automatique) ; ce guide ne concerne QUE le build iOS sur ton Mac.", P))

f.append(Paragraph("1. Ce qui a change cote app (v417)", H2))
...
f.append(Paragraph("- PawPoints (REFONTE v417) : 7 niveaux (Explorateur -> Paw Legend) avec avantages + bonus de points, 2 compteurs (a vie = niveau / depensable = recompenses), echange de points contre reductions (-10/-25/-50%) ou mois gratuits d'abonnement (auto-appliques, 1x/user). Ecran app + page web + boutique. Admin : pages de droite traduites en francais.", B))
...
f.append(Paragraph("APK v417 deja construit et copie dans Downloads : HoPetSit_v23.1.417.apk (95 Mo). AAB Play Store : build/app/outputs/bundle/release/app-release.aab (67 Mo). Permissions FOREGROUND_SERVICE_LOCATION + ACCESS_BACKGROUND_LOCATION deja declarees.", P))
