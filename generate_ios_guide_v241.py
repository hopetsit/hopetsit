"""
HopeTSIT iOS Build Guide v23.1.241 generator.
...
v23.1.241 (20 versions, frontend + minor backend).
"""
...
OUT = "C:/Users/Usuario/Downloads/HopeTSIT_iOS_Build_Guide_v23.1.241.pdf"

doc = SimpleDocTemplate(
    OUT, pagesize=A4,
    leftMargin=1.8*cm, rightMargin=1.8*cm,
    topMargin=1.8*cm, bottomMargin=1.8*cm,
    title="HopeTSIT iOS Build Guide v23.1.241",
...
story.append(Paragraph("Version 23.1.241 - May 27 2026 (release 2)", H2))
...
story.append(Paragraph(
    ...
    "v23.1.241 (push git effectue le 27 mai 2026, commit dbc8b33). v241 "
    "est une release frontend-only : 4 bugs critiques cote chat + friends "
    "(voir section 3) corriges sans toucher l'API. L'API tourne sur "
    ...
    "meme API que l'Android v23.1.241 (deja en prod, APK signe et "
    "fonctionnel).",
    GREEN))
...
story.append(Paragraph(
    ...
    "Xcode + Apple Developer, les changements v23.1.222 -> v23.1.241 "
    "(20 versions cumulees), et le checklist final avant submission "
    "App Store Connect.", P))
story.append(Spacer(1, 0.4*cm))

# ─────────────────────────────────────────────────────────────────────────
# 1. NOUVEAUTES v222 -> v241
# ─────────────────────────────────────────────────────────────────────────
story.append(Paragraph("1. Nouveautes depuis v222 (20 versions)", H2))
story.append(Paragraph(
...
changes = [
    ...
    ("v241",
        "<b>BATCH FOCUS CHAT + FRIENDS (cette release, 4 bugs critiques).</b> "
        "Voir la section 2 ci-dessous pour le detail complet."),
]
...
# 2. v241 DEEP DIVE
# ─────────────────────────────────────────────────────────────────────────
story.append(Paragraph("2. v241 detail (4 bugs corriges)", H2))
story.append(Paragraph(
    "Le batch v241 corrige 4 bugs critiques identifies par Daniel cote "
    "chat + friends. Release frontend-only (aucune modif backend) :",
    P))
...
v240_items = v241_items
_legacy_v240_archived_items = [
...
story.append(Paragraph(
    ...
    "<b>version: 23.1.241+240</b>. Le + suivi du build number est "
...
checklist = [
    ...
    "What's New in This Version : voir bloc ci-dessous (release notes v241)",
...
# ─────────────────────────────────────────────────────────────────────────
story.append(Paragraph("6. Release Notes v241 (a copier-coller App Store)", H2))
story.append(Paragraph(
    "<b>What's New in v23.1.241</b><br/><br/>"
    "Corrections (focus chat + amis, 4 bugs critiques) :<br/>"
    "&bull; Plus de fausse erreur 'Session expiree' dans le chat avec "
    "abonnement PawFollow actif (les messages et demandes s'affichent "
    "enfin correctement)<br/>"
    "&bull; Tap sur un ami dans l'onglet 'Amis' centre la carte sur sa "
    "position avec halo couleur de son metier (violet proprietaire / "
    "vert promeneur / bleu sitter)<br/>"
    "&bull; Toggle 'Partager position' debloque : meme avec PawFollow "
    "actif, tu peux desormais le couper manuellement<br/>"
    "&bull; Onglets 'Chat' et 'Messages' synchronises en temps reel : "
    "memes conversations affichees aux deux endroits<br/>"
    "<br/>"
    "<i>Cette release est un patch frontend-only suite a la v240. "
    "Aucune nouvelle fonctionnalite, uniquement corrections de bugs "
    "remontes par les utilisateurs.</i>",
    P))
story.append(Spacer(1, 0.3*cm))
...
story.append(Paragraph(
    ...
    "version backend deployee (v23.1.241).",
    INFO))
story.append(Spacer(1, 0.3*cm))
...
story.append(Paragraph(
    ...
    "generate_ios_guide_v241.py. Pour toute question, contacter Daniel "
