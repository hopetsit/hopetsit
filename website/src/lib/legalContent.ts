...
export const PRIVACY: LegalDocByLang = {
  en: {
    ...
    { type: "p", html: `Some of our processors are based outside the European Economic Area. Transfers are protected by the European Commission's Standard Contractual Clauses (SCCs) and equivalent safeguards under UK and Hong Kong law.` },
    { type: "h2", html: `7. Cookies` },
    { type: "p", html: `The website uses strictly necessary cookies for authentication and preferences. We do not use advertising or tracking cookies. The mobile app uses local storage and a notification token for push delivery. We measure website traffic ourselves, anonymously and without cookies: no personal data, no local storage, no third-party advertising pixel.` },
    { type: "h2", html: `8. Children` },
    { type: "p", html: `The Service is not directed to children under 16. We do not knowingly collect data from minors.` },
    { type: "h2", html: `9. Changes` },
    { type: "p", html: `Material changes to this policy are notified in-app and by email (when you have opted in to product updates) at least 30 days before they take effect.` },
  ...
  fr: {
    ...
    { type: "h2", html: `7. Cookies` },
    { type: "p", html: `Le site utilise des cookies strictement nécessaires à l'authentification et aux préférences. Nous n'utilisons pas de cookies publicitaires ou de suivi. L'application mobile utilise le stockage local et un jeton de notification pour la livraison push. La fréquentation du site est mesurée par nos soins, de façon anonyme et sans cookie : aucune donnée personnelle, aucun stockage local, aucun pixel publicitaire tiers.` },
  ...
  es: {
    ...
    { type: "p", html: `El sitio web utiliza cookies estrictamente necesarias para la autenticación y las preferencias. No utilizamos cookies publicitarias ni de seguimiento. La aplicación móvil utiliza almacenamiento local y un token de notificación para la entrega automática. Medimos el tráfico del sitio web nosotros mismos, de forma anónima y sin cookies: sin datos personales, sin almacenamiento local y sin píxeles publicitarios de terceros.` },
  ...
  de: {
    ...
    { type: "h2", html: `7. Cookies` },
    { type: "p", html: `Die Website verwendet unbedingt notwendige Cookies zur Authentifizierung und Präferenzen. Wir verwenden keine Werbe- oder Tracking-Cookies. Die mobile App nutzt lokalen Speicher und ein Benachrichtigungstoken für die Push-Zustellung. Die Websitenutzung messen wir selbst, anonym und ohne Cookies: keine personenbezogenen Daten, kein lokaler Speicher, kein Werbe-Pixel Dritter.` },
  ...
  it: {
    ...
    { type: "p", html: `Il sito utilizza cookie strettamente necessari per l'autenticazione e le preferenze. Non utilizziamo cookie pubblicitari o di tracciamento. L'app mobile utilizza l'archiviazione locale e un token di notifica per la consegna push. Misuriamo il traffico del sito internamente, in forma anonima e senza cookie: nessun dato personale, nessuna memoria locale, nessun pixel pubblicitario di terze parti.` },
  ...
  pt: {
    ...
    { type: "p", html: `O site utiliza cookies estritamente necessários para autenticação e preferências. Não utilizamos cookies de publicidade ou rastreamento. O aplicativo móvel usa armazenamento local e um token de notificação para entrega push. Medimos o tráfego do site internamente, de forma anónima e sem cookies: sem dados pessoais, sem armazenamento local e sem pixel publicitário de terceiros.` },
  ...
  ko: {
    ...
    { type: "h2", html: `7. 쿠키` },
    { type: "p", html: `웹사이트는 인증 및 환경설정을 위해 반드시 필요한 쿠키만 사용합니다. 당사는 광고 또는 추적 쿠키를 사용하지 않습니다. 모바일 앱은 로컬 저장소와 푸시 전송을 위한 알림 토큰을 사용합니다. 당사는 웹사이트 방문 통계를 쿠키 없이 익명으로 직접 측정합니다. 개인정보, 로컬 저장소, 제3자 광고 픽셀을 사용하지 않습니다.` },
    { type: "h2", html: `8. 아동` },
  ...
  ja: {
    ...
    { type: "h2", html: `7. Cookie` },
    { type: "p", html: `本ウェブサイトは、認証および設定のために厳密に必要なCookieのみを使用します。当社は広告用または追跡用のCookieを使用しません。モバイルアプリは、ローカルストレージとプッシュ配信のための通知トークンを使用します。ウェブサイトのアクセス状況は、Cookieを使わず匿名で当社自身が測定しています。個人データ、ローカルストレージ、第三者の広告ピクセルは一切使用しません。` },
    { type: "h2", html: `8. 未成年者` },
